...
%	\end{frame}
	...
	\begin{frame}
		First equation of \eqref{transformed-DAE-const-coeff} is a first order ordinary differential equation and posesses a unique solution $u(t)$ in $[t_0,t_l]$ for any initial values $u_0 \in \mathbb{R}^d$. 
		Now let $q(t) \in C^{k-1}([t_0,t_l])$ and differentiate the second equation in \eqref{transformed-DAE-const-coeff}:
		\begin{align*}
			v(t) &= q(t) - Nv'(t) = q(t) - N(\underbrace{q(t)-Nv'(t)}_{=v(t)})' = q-Nq'+N^2v'' \\
			&= q-Nq'+N^2(q-Nv')'' = q-Nq'+N^2q''-N^3v''' \\
			&\vdots \\
		...
		(There exist other index concepts, for our case these are all equal.)
	\end{frame}
