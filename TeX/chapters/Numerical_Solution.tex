...
%	\end{theorem}
	...
	\begin{theorem}[\protecting{\cite[Satz~4.2.10]{NumerikGewöhnlicherDifferentialgleichungen}}]
		...
		Let the linear multi-step method be zero-stable and consistent of order $p$, then it is also convergent of order $p$.
	\end{theorem}
	
	equivalent to lax richtmyer in single step
	
	am streifen S, was ist f genau?
...
- we are linear, thus no newton needed, only for nonlinear - numerik buch auch mit nur zwei schritten - adams lin mehrschrittverfahre besteht aus usw.
...
\section{Numerical Examples}
	...
	für tabellen anschauen:
	SI - unit x - packet
	csvsimple for tables
	...
	\begin{center}	
		...
		{tables/err_ex2.csv}{h=\h, oneu=\oneu, onei=\onei, twou=\twou, twoi=\twoi, threeu=\threeu, threei=\threei, trapu=\trapu, trapil=\trapil}
		{\h & \num{\oneu} & \num{\onei} & \num{\twou} & \num{\twoi} & \num{\threeu} & \num{\threei} & \num{\trapu} & \num{\trapil}}
	\end{center}
