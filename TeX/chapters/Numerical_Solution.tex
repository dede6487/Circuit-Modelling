...
%	\end{theorem}
	...
	To achieve these convergent rates it is also important to compute the initial steps for the multi-step method appropriately, i.e. the methods with which these initial steps are calculated have to also be of the same order as we would want our solution to be. (In applications this can mean that we calculate the initial steps with methods of lower order, but using a smaller step-size)
...
\begin{example}
	...
	The numerical methods result in errors displayed in Table \ref{tab:error ex3}. Due to the fact, that $u_1$ is again the same as $-v_src$, it is not of interest for our comparison of the errors. We omit $u_1$.
