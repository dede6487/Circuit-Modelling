...
\section{Multistep-Methods}
	...
	\begin{definition}
		Let $\tilde{y}_{m+k}$ be the result of one step of the multi-step method \eqref{linear-multistep-method} with the start-values given as the exact results of the solution $y_{m+l} = y(t_m+l)$, $0 \leq l < k$. This means
		\begin{displaymath}
...
%		k = 6 &: h f_{m+6} = \frac{1}{60} (147 u_{m+6} - 360 u_{m+5} + 450 u_{m+4} - 400 u_{m+3} + 225 u_{m+2} - 72 u_{m+1} + 10 u_m)
	...
	Methods with $s > 6$ are not zero-stable. Indeed in practice, methods with order greater than 3 are rarely used because of their low smoothness properties.
